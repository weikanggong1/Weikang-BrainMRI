...
jobs = [
    {
        "task": "synthsr",
        "model": {"weights": os.environ["SR_WEIGHT"]},
        "kwargs": {"image": f"examples/wmh_data/sub-{n}_FLAIR.nii.gz"},
        "outputs": {"image": str(folder / f"sub-{n}_synthsr.nii.gz")},
    }
    for n in ("02", "03", "04")
]
...
rows = json.loads(Path("work/synthsr_public_batch/report.json").read_text())
assert len(rows) == 3
assert all(row["error"] is None and Path(row["outputs"]["image"]).is_file()
           for row in rows)
assert {row["device"] for row in rows} == {"cuda:0", "cuda:1"}
assert len({row["pid"] for row in rows}) == 2
assert any(
    a["pid"] != b["pid"]
    and max(a["started_at"], b["started_at"])
        < min(a["finished_at"], b["finished_at"])
    for i, a in enumerate(rows) for b in rows[i + 1:]
)
print("3 cases, 2 GPU workers, overlapping execution")
PY
```

重新运行时，已有输出会触发冲突检查；改用新目录，或显式加 `--overwrite`。

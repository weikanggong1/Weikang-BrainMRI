"""Trace the blurred atlas mean-frame through surface reparameterization."""
...
def main() -> None:
    ...
    atlas = tifffile.imread(args.atlas)[3].view(np.float32)
    native = np.asanyarray(nib.load(str(args.native_mgz)).dataobj)[:, :, 3].T
    start = perf_counter()
    ...
    blurred = blur_atlas_frame(torch.from_numpy(atlas.copy()).to(args.device), args.sigma)

...
def matrix(offset, rows, cols):
    ...
    return result
a, b, c = read(row + 4, "<3f")
print("RH83_LINE_FIT " + json.dumps({"dt_in":read(rsp+0xe0,"<3d"), "sse_out":read(rsp+0x400,"<3d"), "normal_matrix":matrix(0xa0,3,3), "normal_inverse":matrix(0xb0,3,3), "right_hand_side":matrix(0xc0,3,1), "a":a,"b":b,"c":c,"candidate":struct.unpack("<f",struct.pack("<f",-b/a))[0]}))
end
call (int) fflush(0)
quit

...
/* Search for the first voxel in vol which has the specified value. The
 * search begins from the centre of the volume, and extends outwards in a
 * recti-linear manner.
 *
 * If no voxel with the specified value can be found, an empty vector is
 * returned. Otherwise the vector will contain the XYZ coordinates.
 */
template<class T>
std::vector<int64_t> search_from_centre(const volume<T> vol,
                                        const T value) {
  ...
  while (true) {
    ...
    xlo = std::max(xmin, xmid - radius);
    ylo = std::max(ymin, ymid - radius);
    zlo = std::max(zmin, zmid - radius);
    xhi = std::min(xmax, xmid + radius);
    yhi = std::min(ymax, ymid + radius);
    zhi = std::min(zmax, zmid + radius);
    ...
    for (z=zstart; z <= zhi; z++) {
      for (y=ystart; y <= yhi; y++) {
        for (x=xstart; x <= xhi; x++) {
          if (vol(x, y, z) == value) {
            loc.push_back(x);
            loc.push_back(y);
            loc.push_back(z);
...
volume<float> fill_head_mask(const volume<float>& origmask)
{
  ...
  label = connected_components(mask + 1, 6);

  // Identify the label of the brain
  // component (where mask == 1)
  std::vector<int64_t> brainloc = search_from_centre(mask, (float)1);

  // brain mask has no voxels
  // with a value of 1??
  if (brainloc.size() == 0) {
    return mask;
  }
  int brainlbl = label(brainloc[0], brainloc[1], brainloc[2]);

  // Get the labels of every component
  // that touches a corner of the volume
  std::set<int> cornerlbls;
  cornerlbls.insert(label(label.minx(), label.miny(), label.minz()));
  cornerlbls.insert(label(label.minx(), label.miny(), label.maxz()));
  cornerlbls.insert(label(label.minx(), label.maxy(), label.minz()));
  cornerlbls.insert(label(label.minx(), label.maxy(), label.maxz()));
  cornerlbls.insert(label(label.maxx(), label.miny(), label.minz()));
  cornerlbls.insert(label(label.maxx(), label.miny(), label.maxz()));
  cornerlbls.insert(label(label.maxx(), label.maxy(), label.minz()));
  cornerlbls.insert(label(label.maxx(), label.maxy(), label.maxz()));

  // Generate a new brain mask, where
  // the background is the union of all
  // components which touch every
  // corner of the image (minus the
  // brain component, if applicable),
  // and the brain is everything else.
  cornerlbls.erase(brainlbl);
  for (int z = label.minz(); z < label.maxz(); z++) {
    for (int y = label.miny(); y < label.maxy(); y++) {
      for (int x = label.minx(); x < label.maxx(); x++) {

        // brain
        if (cornerlbls.find(label(x, y, z)) == cornerlbls.end()) {
          mask(x, y, z) = 1;
        }
        // background
        else {

...
std::shared_ptr<MISCMATHS::BFMatrix> IntensityMapper::Hessian(const NEWIMAGE::volume<float>&    ref,
                                                                const NEWIMAGE::volume<char>      *mask,
...
const
{
  ...
  std::shared_ptr<MISCMATHS::BFMatrix>   hess(new MISCMATHS::FullBFMatrix(0,0));  // Empty matrix
  return(hess);
}
...
std::shared_ptr<MISCMATHS::BFMatrix> IntensityMapper::CrossHessian(const BASISFIELD::basisfield&        dfield,
...
std::shared_ptr<MISCMATHS::BFMatrix> SSDIntensityMapper::Hessian(const NEWIMAGE::volume<float>&    ref,
                                                                   const NEWIMAGE::volume<char>      *mask,
                                                                   MISCMATHS::BFMatrixPrecisionType  prec)
const
{
  ...
  std::shared_ptr<MISCMATHS::BFMatrix>   hess; // Null-pointer
  if (Fixed()) {
    hess = std::shared_ptr<MISCMATHS::BFMatrix>(new MISCMATHS::FullBFMatrix(0,0));  // Empty matrix
    return(hess);
  }
  ...
  if (_mt == NO_SCALING) {
    ...
    hess = std::shared_ptr<MISCMATHS::BFMatrix>(new MISCMATHS::FullBFMatrix(1,1));
    hess->Set(1,1,tmp);
  }
  else if (_mt == GLOBAL_NON_LINEAR) {
    ...
    hess = std::shared_ptr<MISCMATHS::BFMatrix>(new MISCMATHS::FullBFMatrix(FtF));
  }
  else if (_mt == LOCAL_LINEAR) {
    std::shared_ptr<MISCMATHS::BFMatrix>   reg = _sfld[0]->BendEnergyHess(prec);
    ...
    std::vector<std::vector<std::shared_ptr<MISCMATHS::BFMatrix> > >  JtJ(_sfld.size());
    ...
    for (unsigned int i=0; i<_sfld.size(); i++) {
      ...
      for (unsigned int j=i; j<_sfld.size(); j++) {
        if (j==i) {
          std::shared_ptr<MISCMATHS::BFMatrix>   reg = _sfld[i]->BendEnergyHess(prec);
          reg->MulMeByScalar(0.5 * _lambda);
...
std::shared_ptr<MISCMATHS::BFMatrix> SSDIntensityMapper::CrossHessian(const BASISFIELD::basisfield&        dfield,
                                                                        const NEWIMAGE::volume<float>&       dima,
                                                                        const NEWIMAGE::volume<float>&       ref,
                                                                        const NEWIMAGE::volume<char>         *mask,
                                                                        MISCMATHS::BFMatrixPrecisionType     prec)
const
{
  ...
  if (_mt == NO_SCALING) {
    ...
    hess = std::shared_ptr<MISCMATHS::BFMatrix>(new MISCMATHS::FullBFMatrix(-dfield.Jte(dima,ref,mask)));  // N.B. the minus sign
  }
  else if (_mt == GLOBAL_NON_LINEAR) {
    ...
    hess = std::shared_ptr<MISCMATHS::BFMatrix>(new MISCMATHS::FullBFMatrix(JtF));
  }
  else if (_mt == LOCAL_LINEAR) {
...
vector<std::shared_ptr<basisfield> > IntensityMapperReader::GetLocalAsSplinefieldVector(const vector<unsigned int>& ksp) const
{
  if (!_has_local) throw IntensityMapperReaderException("GetLocalAsSplinefieldVector: No local intensity mapping info available.");
  vector<std::shared_ptr<basisfield> >  rvec(_local.tsize());
  ...
  for (int i=0; i<_local.tsize(); i++) {
    rvec[i] = std::shared_ptr<basisfield>(new splinefield(size,vxs,ksp));
    rvec[i]->Set(_local[i]);
  }
  return(rvec);
}

...
fnirt_CF::fnirt_CF(const NEWIMAGE::volume<float>&                                  pvref,
                   ...
                   std::vector<std::shared_ptr<BASISFIELD::basisfield> >&        pdf_field,
                   std::shared_ptr<IntensityMapper>                              im)
...
fnirt_CF::fnirt_CF(const std::shared_ptr<NEWIMAGE::volume<float> >               pvref,
                   const std::shared_ptr<NEWIMAGE::volume<float> >               pvobj,
                   const NEWMAT::Matrix&                                           paff,
                   std::vector<std::shared_ptr<BASISFIELD::basisfield> >&        pdf_field,
                   std::shared_ptr<IntensityMapper>                              im)
...
void fnirt_CF::common_construction(std::vector<std::shared_ptr<BASISFIELD::basisfield> >& pdf_field)
{
  ...
  if (!imap) {
    imap = std::shared_ptr<IntensityMapper>(new IntensityMapper);  // NO_SCALING if no imap was specified
  }
  ...
  robj = std::shared_ptr<NEWIMAGE::volume<float> >(new NEWIMAGE::volume<float>(RefSz_x(),RefSz_y(),RefSz_z()));
  ...
  *robj = 0.0;
  ...
  datamask = std::shared_ptr<NEWIMAGE::volume<char> >(new NEWIMAGE::volume<char>(RefSz_x(),RefSz_y(),RefSz_z()));
  copybasicproperties(*ssvref,*datamask);
  // df is a volume representation of the field
  df = std::shared_ptr<NEWIMAGE::volume<float> >(new NEWIMAGE::volume<float>(RefSz_x(),RefSz_y(),RefSz_z(),df_field.size()));
...
void fnirt_CF::SubsampleRef(const std::vector<unsigned int>& ss)
{
  if (ss != subsamp) { // If anything has really changed
    ...
    // Resample field
    for (unsigned int i=0; i<df_field.size(); i++) {
      std::shared_ptr<BASISFIELD::basisfield> tmp = df_field[i]->ZoomField(ss2ms(ss),ss2vxs(ss));
      df_field[i] = tmp;
    }
    ...
    df = std::shared_ptr<NEWIMAGE::volume4D<float> >(new NEWIMAGE::volume4D<float>(RefSz_x(),RefSz_y(),RefSz_z(),df_field.size()));
    ...
    robj_updated = false;
    robjmask = std::shared_ptr<NEWIMAGE::volume<char> >(new NEWIMAGE::volume<char>(RefSz_x(),RefSz_y(),RefSz_z()));
    ...
    if (robj_deriv) {
      robj_deriv = std::shared_ptr<NEWIMAGE::volume4D<float> >();
      robj_deriv_updated = false;
    }

    // Resize totmask
    if (totmask) {
      totmask = std::shared_ptr<NEWIMAGE::volume<char> >(new NEWIMAGE::volume<char>(RefSz_x(),RefSz_y(),RefSz_z()));
...
void fnirt_CF::SmoothRef(double fwhm)
{
  if (fwhm != ref_fwhm) {
    ...
    if (!fwhm) {
      ...
      svref = std::shared_ptr<NEWIMAGE::volume<float> >(new NEWIMAGE::volume<float>(smooth(*vref,fwhm/sqrt(8.0*log(2.0)))));
    }
...
void fnirt_CF::IntensityScaleObj(double sfac)
{
  *vobj *= sfac;      // Scale
  ...
  if (obj_fwhm) {     // If there is a smoothed version, smooth scaled image
    ...
    // svobj = std::shared_ptr<NEWIMAGE::volume<float> >(new NEWIMAGE::volume<float>(smooth(*vobj,obj_fwhm/sqrt(8.0*log(2.0)))));
  }
  robj_updated = false;
}
...
void fnirt_CF::SmoothObj(double fwhm)
{
  if (fwhm != obj_fwhm) {
    ...
    if (!fwhm) {
      ...
      // svobj = std::shared_ptr<NEWIMAGE::volume<float> >(new NEWIMAGE::volume<float>(smooth(*vobj,fwhm/sqrt(8.0*log(2.0)))));
    }
    robj_updated = false;
  }
...
{
  const std::shared_ptr<NEWIMAGE::volume<char> >  tmp = std::shared_ptr<NEWIMAGE::volume<char> >(new NEWIMAGE::volume<char>(mask));
  SetRefMask(tmp);
}

void fnirt_CF::SetRefMask(const std::shared_ptr<NEWIMAGE::volume<char> > mask)
{
...
void fnirt_CF::SetObjMask(const std::shared_ptr<NEWIMAGE::volume<char> > mask)
{
...
const NEWIMAGE::volume4D<float>& fnirt_CF::RobjDeriv() const
{
  if (!robj_deriv) { // If first time derivative of object image is requested
    robj_deriv = std::shared_ptr<NEWIMAGE::volume4D<float> >(new NEWIMAGE::volume4D<float>(RefSz_x(),RefSz_y(),RefSz_z(),3));
...
{
  if (!ref_deriv) {
    ref_deriv = std::shared_ptr<NEWIMAGE::volume4D<float> >(new NEWIMAGE::volume4D<float>(RefSz_x(),RefSz_y(),RefSz_z(),3));
...
void fnirt_CF::SetDefFieldParams(const NEWMAT::ColumnVector& p) const
{
  ...
  bool sameasold = true;
  for (unsigned int i=0; i<df_field.size(); i++) {
    std::shared_ptr<NEWMAT::ColumnVector>  oldp = df_field[i]->GetCoef();  // Shared ptr must be named variable
...
{
  ssvref = std::shared_ptr<NEWIMAGE::volume<float> >(new NEWIMAGE::volume<float>(int(nms[0]),int(nms[1]),int(nms[2])));
...
{
  if (refmask) {
    ssrefmask = std::shared_ptr<NEWIMAGE::volume<char> >(new NEWIMAGE::volume<char>(int(nms[0]),int(nms[1]),int(nms[2])));
...
std::shared_ptr<NEWIMAGE::volume<float> > fnirt_CF::masked_smoothing(const NEWIMAGE::volume<float>&              vol,
                                                                       double                                      fwhm,
                                                                       std::shared_ptr<NEWIMAGE::volume<char> >  mask)
{
  std::shared_ptr<NEWIMAGE::volume<float> > tmpvol = std::shared_ptr<NEWIMAGE::volume<float> >(new NEWIMAGE::volume<float>(vol));
  if (mask) {
...
std::shared_ptr<MISCMATHS::BFMatrix> SSD_fnirt_CF::hess(const NEWMAT::ColumnVector&             p,
                                                          std::shared_ptr<MISCMATHS::BFMatrix>  iptr) const
{
  //
  ...
  if (last_hess && UsingRefDeriv()) {               // If we are using derivatives from reference scan
    if (int(last_hess->Ncols()) == p.Nrows()) {     // If there has been no zooming
      ...
      last_hess->Clear();
      last_hess = std::shared_ptr<MISCMATHS::BFMatrix>();
    }
  }
  ...
  //
  // Diagonal blocks
  std::shared_ptr<MISCMATHS::BFMatrix> dxTdx = DefField(0).JtJ(partial[0],&mask,HessianPrecision());
  std::shared_ptr<MISCMATHS::BFMatrix> dyTdy = DefField(0).JtJ(partial[1],&mask,HessianPrecision());
  std::shared_ptr<MISCMATHS::BFMatrix> dzTdz = DefField(0).JtJ(partial[2],&mask,HessianPrecision());
  dxTdx->MulMeByScalar(2.0/double(n));
  dyTdy->MulMeByScalar(2.0/double(n));
  dzTdz->MulMeByScalar(2.0/double(n));

  // Off-diagonal blocks
  std::shared_ptr<MISCMATHS::BFMatrix> dxTdy = DefField(0).JtJ(partial[1],partial[0],&mask,HessianPrecision());
  std::shared_ptr<MISCMATHS::BFMatrix> dxTdz = DefField(0).JtJ(partial[2],partial[0],&mask,HessianPrecision());
  std::shared_ptr<MISCMATHS::BFMatrix> dyTdz = DefField(0).JtJ(partial[2],partial[1],&mask,HessianPrecision());
  ...
  if (MPL()) {
    std::shared_ptr<MISCMATHS::BFMatrix> mplH = MPL()->SSD_Hessian(0,DefField(0),HessianPrecision());
  ...
  std::shared_ptr<MISCMATHS::BFMatrix> imapH = IMap()->Hessian(ref,&mask,HessianPrecision());
  imapH->MulMeByScalar(2.0/double(n));

  // Calculate Cross-term between spatial and intensity mapping.
  std::shared_ptr<MISCMATHS::BFMatrix> xCross = IMap()->CrossHessian(DefField(0),partial[0],ref,&mask,HessianPrecision());
  xCross->MulMeByScalar(2.0/double(n));
  std::shared_ptr<MISCMATHS::BFMatrix> yCross = IMap()->CrossHessian(DefField(0),partial[1],ref,&mask,HessianPrecision());
  yCross->MulMeByScalar(2.0/double(n));
  std::shared_ptr<MISCMATHS::BFMatrix> zCross = IMap()->CrossHessian(DefField(0),partial[2],ref,&mask,HessianPrecision());

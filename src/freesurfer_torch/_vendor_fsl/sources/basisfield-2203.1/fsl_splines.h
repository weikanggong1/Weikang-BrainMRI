...
private:
  std::string m_msg;
public:
  FslSplinesException(const std::string& msg) noexcept : m_msg(std::string("FslSplines::" + msg)) {}
...
public:
  ...
  // Functions returning the total number of splines that has an overlap
  // with that given by cindx. The first form assumes that "this" spline
  // has the same knot-spacing as those it overlaps with. The second form
  // allows for passing a different spline by which "this" overlaps.
  // Useful for finding the number of non-zero elements in a column
  // (given by cindx) of AtA (first form) or AtB (second form).

  unsigned int NumberOfOverlappingSplines(const std::vector<unsigned int>& cindx,
                          ...
                          const Spline3D<T>&                sp2) const
{
  ...
  for (unsigned int d=0; d<3; d++) {
    ...
    f1[d] = std::max(static_cast<int>(if2)-static_cast<int>(if1),0) + o1;
    f2[d] = std::max(static_cast<int>(if1)-static_cast<int>(if2),0) + o2;
    l1[d] = std::min(il1,il2) - if1 + o1;
  }
...
/////////////////////////////////////////////////////////////////////
//
// Get number of splines that overlap with *this. Useful for working
// out number of non-zero elements per column of AtA/AtB.
//
/////////////////////////////////////////////////////////////////////

template<class T>
unsigned int Spline3D<T>::NumberOfOverlappingSplines(const std::vector<unsigned int>& cindx,
						     const std::vector<unsigned int>& isz) const
{
  unsigned int nos = 1;
  unsigned int first = 0, last = 0;
  for (unsigned int i=0; i<3; i++) {
    if (!_vsp1D[i].RangeOfOverlappingSplines(cindx[i],isz[i],first,last)) return(0);
    else nos *= last-first;
  }
  return(nos);
}
						     ...
						     const Spline3D<T>&               sp2) const
{
  ...
  unsigned int first = 0, last = 0;
  for (unsigned int i=0; i<3; i++) {
    if (!_vsp1D[i].RangeOfOverlappingSplines(cindx[i],isz[i],sp2._vsp1D[i],first,last)) return(0);
    else nos *= last-first;
  }
  return(nos);
}
...
{
  if (_ksp==1) {
    first = std::max(0,static_cast<int>(cindx) - static_cast<int>(2*(_order/2)));
    last = std::min(NCoef(isz),cindx+2*(_order/2)+1);
  }
  else {
    first = std::max(0,static_cast<int>(cindx) - static_cast<int>(_order));
    last = std::min(NCoef(isz),cindx+_order+1);
  }
                                            ...
                                            unsigned int&         last) const  // Last coefficient/spline with overlap
{
  ...
  for (unsigned int i=0; i<sp2.NCoef(isz); i++) {
    ...
    int overlap = std::min(lme-1,lsp2-1) - std::max(fme,fsp2);

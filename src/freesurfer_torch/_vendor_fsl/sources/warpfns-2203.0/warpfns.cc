...
std::vector<unsigned int> rows_per_thread(unsigned int nrows,
					  unsigned int nthr)
{
  double rpt = static_cast<double>(nrows) / static_cast<double>(nthr);
  std::vector<unsigned int> nr(nthr+1,0);
  for (unsigned int i=1; i<nthr; i++) nr[i] = static_cast<unsigned int>(std::floor(i*rpt));
  nr[nthr] = nrows;
  return(nr);
}

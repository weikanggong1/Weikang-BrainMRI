/*  applywarp.cc
    ...
    Mark Jenkinson, Matthew Webster and Jesper Andersson, FMRIB Image Analysis Group
...
// By virtue of including warpfns.h before including newimage.h we should
// have access to all the "EXPOSE_TREACHEROUS" parts of newimage.
...
public:
  ...
  // Checks s/qforms are the same and returns it
  static NEWMAT::Matrix get_and_confirm_s_qform(const NEWIMAGE::volume<float>& vol,
						const std::string&             fname);

  // Create the affine matrix that can be used by applywarp to go from invol
  // to refvol based on the sform matrices.
  static NEWMAT::Matrix make_affmat_from_sforms(const NEWIMAGE::volume<float>& invol,
						const NEWMAT::Matrix&          insform,
						const NEWIMAGE::volume<float>& refvol,
						const NEWMAT::Matrix&          refsform);
};
...
string examples=string("applywarp -i invol -o outvol -r refvol -w warpvol\n") +
                ...
                string("applywarp -i invol -o outvol -r refvol --usesqform\n");
...
Option<string> interp(string("--interp"), string(""),
		   string("interpolation method {nn,trilinear,sinc,spline} (default: trilinear)"),
...
Option<int> padsize(string("--paddingsize"), 0,
		       ...
		       false, requires_argument);
Option<bool> usesqform(string("--usesqform"), false,
		       string("use s/qforms of --ref and --in images"),
		       false, no_argument);
...
int ApplyWarpHelper::applywarp()
{
  ...
  if (usesqform.set()) { // Cannot be combined with any other transforms
    if (prematname.set() || postmatname.set() || warpname.set()) {
      cerr << "The --usesqform option cannot be used with any other transforms" << endl;
      exit(EXIT_FAILURE);
    }
  }
  ...
  for (int i=0; i<invol.tsize(); i++) outvol.addvolume(refvol[0]);
  ...
  else if (usesqform.set()) { // This is intended to let us resample --in to --ref based on their s/qforms
    wt = RelativeWarps;
    defvol.setdims(refvol.xdim(),refvol.ydim(),refvol.zdim(),1.0);
    defvol = 0.0;
    Matrix inform = ApplyWarpHelper::get_and_confirm_s_qform(invol[0],inname.value());
    Matrix refform = ApplyWarpHelper::get_and_confirm_s_qform(refvol[0],refname.value());
    affmat = ApplyWarpHelper::make_affmat_from_sforms(invol[0],inform,refvol[0],refform);
  }
  ...
  std::shared_ptr<volume<float> >   ssout_ptr;     // Used for (optional) super-sampling
  if (superflag) {
    ...
    ssout_ptr = std::shared_ptr<volume<float> >(new volume<float>(isize[0],isize[1],isize[2]));
...
// "undo" some of the things that take place in there.
NEWMAT::Matrix ApplyWarpHelper::make_affmat_from_sforms(const NEWIMAGE::volume<float>& invol,
							const NEWMAT::Matrix&          insform,
							const NEWIMAGE::volume<float>& refvol,
							const NEWMAT::Matrix&          refsform)
{
  return((invol.sampling_mat()*insform.i()*refsform*refvol.sampling_mat().i()).i());
}

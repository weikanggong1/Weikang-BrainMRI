...
: _pevec(pevec), _rotime(rotime), _uptodate(false), _tp(false), _nthr(nthreads._n)
{
  ...
  _orig = std::shared_ptr<NEWIMAGE::volume<float> >(new NEWIMAGE::volume<float>(scan));
                                                         ...
                                                         const BASISFIELD::splinefield& field) const
{
  ...
  for (int k=0; k<rval.zsize(); k++) {
	...
	NEWMAT::ColumnVector y2 = T2*x;
        ...
        rval(i,j,k) = float(sf[0]*(y2(1)-y1(1))*_derivs(i,j,k,0) + sf[1]*(y2(2)-y1(2))*_derivs(i,j,k,1) + sf[2]*(y2(3)-y1(3))*_derivs(i,j,k,2));
      }
    }
  }
  rval /= tiny;
                                                                  ...
                                                                  const BASISFIELD::splinefield& field) const
{
  ...
  resampled2 = 0.0;
  general_transform(*_smooth,rb,df,resampled2,Utilities::NoOfThreads(_nthr));
...
void TopupScan::Smooth(double fwhm)
{
  ...
  if (fwhm == 0.0) {
    ...
    if (_smooth == _subsamp) { // If no copy made yet
      _smooth = std::shared_ptr<NEWIMAGE::volume<float> >(new NEWIMAGE::volume<float>(_subsamp->xsize(),_subsamp->ysize(),_subsamp->zsize()));
    }
    copybasicproperties(*_subsamp,*_smooth);
    *_smooth = NEWIMAGE::smooth(*_subsamp,fwhm/sqrt(8.0*log(2.0)));
    _smooth->forcesplinecoefcalculation();
  }
  _uptodate = false;
...
void TopupScan::SubSample(unsigned int ss)
{
  ...
  if (ss == 1) {
    ...
    _subsamp = std::shared_ptr<NEWIMAGE::volume<float> >(new NEWIMAGE::volume<float>(_regrid->xsize()/ss,_regrid->ysize()/ss,_regrid->zsize()/ss));
...
void TopupScan::ReGrid(int xsz, int ysz, int zsz)
{
  ...
  if (xsz == _orig->xsize() && ysz == _orig->ysize() && zsz == _orig->zsize()) {
    ...
    _regrid = std::shared_ptr<NEWIMAGE::volume<float> >(new NEWIMAGE::volume<float>(xsz,ysz,zsz));
...
void TopupScan::update(const BASISFIELD::splinefield&  field) const
{
  ...
  if (!_uptodate) {
    ...
    general_transform_3partial(*_smooth,rb,df,_resampled,_derivs,_mask,Utilities::NoOfThreads(_nthr));
    ...
    if ((_pevec(1) && !_pevec(2)) || (!_pevec(1) && _pevec(2))) { // If warps only in one direction
      BASISFIELD::splinefield dfield = field;
      double sf = (_pevec(1)) ? _rotime * _pevec(1) * _orig->xdim() : _rotime * _pevec(2) * _orig->ydim();
      dfield.ScaleField(sf);
      BASISFIELD::FieldIndex fi = (_pevec(1)) ? BASISFIELD::FieldIndex(1) : BASISFIELD::FieldIndex(2);
      NEWIMAGE::deffield2jacobian(dfield,fi,_jac);
    }
    else {
...
TopupScanManager::TopupScanManager(const NEWIMAGE::volume4D<float>&   scans,
				   ...
				   Utilities::NoOfThreads             nthreads)
: _ss(1), _fwhm(0.0), _mpi(scans.tsize()), _tp(false), _nthr(nthreads._n)
{
  ...
  for (int i=0; i<scans.tsize(); i++) {
    _scans[i] = new TopupScan(scans[i],pevecs.Column(i+1),rotimes(i+1),nthreads);
  }
...
TopupCF::TopupCF(const NEWIMAGE::volume4D<float>&     scans,
		 ...
		 Utilities::NoOfThreads               nthreads)
: _sm(scans,pevecs,rotimes,nthreads), _field(field_factory(scans,warpres,sporder,nthreads)), _wr(warpres), _lambda(10), _ssql(true), _rt(BendingEnergy), _mf(false), _hp(MISCMATHS::BFMatrixDoublePrecision), _nthr(nthreads._n), _dl(0), _level(0), _iter(0), _attempt(0)
{
...
void TopupCF::SubSample(unsigned int ss)
{
  ...
  if (ss != _sm.SubSampling()) {
    ...
    BASISFIELD::splinefield new_field(size,vxs,ksp,_field.Order(),Utilities::NoOfThreads(_nthr));
...
std::shared_ptr<MISCMATHS::BFMatrix> TopupCF::FieldEnergyHess() const
{
  if (TracePrint()) cout << "Entering TopupCF::FieldEnergyHess" << endl;

  std::shared_ptr<MISCMATHS::BFMatrix> hptr;
...
std::shared_ptr<MISCMATHS::BFMatrix> TopupCF::hess(const NEWMAT::ColumnVector&             p,
                                                   std::shared_ptr<MISCMATHS::BFMatrix>  iptr) const
{
  ...
  std::shared_ptr<MISCMATHS::BFMatrix> nonlin_bit = _field.JtJ(*(abc[0]),ones,&mask,HessianPrecision());
  std::vector<unsigned int> deriv1(3,0);
  std::vector<unsigned int> deriv2(3,0);
  if (_sm.HasBeta()) {
    deriv2[0] = 1;
    std::shared_ptr<MISCMATHS::BFMatrix> tmp = _field.JtJ(deriv1,*(abc[1]),deriv2,ones,&mask,HessianPrecision());
  ...
  if (_sm.HasGamma()) {
    deriv2[1] = 1;
    std::shared_ptr<MISCMATHS::BFMatrix> tmp = _field.JtJ(deriv1,*(abc[2]),deriv2,ones,&mask,HessianPrecision());
    nonlin_bit->AddToMe(*tmp);
    nonlin_bit->AddToMe(*(tmp->Transpose()));
  ...
  if (_sm.HasBeta() && _sm.HasGamma()) {
    deriv1[0] = 1;
    deriv2[1] = 1;
    std::shared_ptr<MISCMATHS::BFMatrix> tmp = _field.JtJ(deriv1,*(abc[4]),deriv2,ones,&mask,HessianPrecision());
    nonlin_bit->AddToMe(*tmp);
    nonlin_bit->AddToMe(*(tmp->Transpose()));
  }
  ...
  if (!MovementsFixed()) {
    ...
    for (unsigned int s1=1; s1<_sm.NoOfScans(); s1++) {
      for (unsigned int s2=s1; s2<_sm.NoOfScans(); s2++) {
        if (s1 == s2) {
	  tiled_move_bit.SetTile(s1,s2,((2.0/(n*(m-1))) * (1.0 - 1.0/m)) * movement_hessian(s1,s2,mask,_field));
	}
        else {
    ...
    std::shared_ptr<MISCMATHS::BFMatrix> move_bit = std::shared_ptr<MISCMATHS::BFMatrix>(new FullBFMatrix(tiled_move_bit.Untile()));
    ...
    std::shared_ptr<MISCMATHS::BFMatrix> bf_interaction = std::shared_ptr<MISCMATHS::BFMatrix>(new FullBFMatrix(interaction));
...
BASISFIELD::splinefield TopupCF::field_factory(const NEWIMAGE::volume4D<float>& scans,
                                               double                           warpres,
                                               unsigned int                     sporder,
					       Utilities::NoOfThreads           nthreads) const
{
  std::vector<unsigned int>  size(3,0);
  ...
  BASISFIELD::splinefield field(size,vxs,ksp,sporder,nthreads);

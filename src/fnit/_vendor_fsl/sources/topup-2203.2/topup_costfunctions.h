...
{
public:
  TopupException(const std::string& msg) noexcept : m_msg(std::string("Topup::") + msg) {}
  virtual const char * what() const noexcept { return(m_msg.c_str()); }
  ~TopupException() noexcept {}
private:
  std::string m_msg;
};
...
{
public:
  TopupScan(const NEWIMAGE::volume<float>& scan, const NEWMAT::ColumnVector& pevec, double rotime, Utilities::NoOfThreads nthreads);
  ~TopupScan() {}
...
private:
  ...
  unsigned int                                  _nthr;      // No of threads
...
private:
  ...
  bool                                          _hasy;
  bool                                          _tp;     // Trace-print
  unsigned int                                  _nthr;   // Number of threads
...
public:
  ...
  unsigned int NThreads() const { return(_nthr); }
...
private:
  ...
  unsigned int                      _nthr;    // Number of threads to run with
  bool                              _verb;    // Verbose
  bool                              _tp;      // Trace-print

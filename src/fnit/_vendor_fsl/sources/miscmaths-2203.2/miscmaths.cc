...
#define BINFLAG 42
  ...
  ReturnMatrix pinv(const Matrix& input) {
    ...
    Matrix result;
    try {
      result=arma::pinv(input.get_at_ref(),0);
    }
    catch (...) {
      std::cerr << std::endl << "Error during MISCMATHS::pinv using dc, attempting once more with std." << std::endl;
      result=arma::pinv(input.get_at_ref(),0,"std");
    }
    result.Release();
    return result;
  }

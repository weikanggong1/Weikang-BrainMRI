...
{
public:
  fnirt_error(const std::string& pmsg) throw() : msg(pmsg) {}
  const char *what() const throw() {return(std::string("fnirt::" + msg).c_str());}
  ~fnirt_error() throw() {}
private:
  std::string msg;
};
...
private:
  ...
  bool                                         verbose;
  unsigned int                                 debug;
  MISCMATHS::BFMatrixPrecisionType             hess_prec;
...
public:
  ...
  const std::string& CoefFname() const {
    ...
    if (!coef.length()) coef = FNIRT::path(obj) + FNIRT::filename(obj) + std::string("_warpcoef");
    return(coef);
  }
  const std::string& LogFname() {
    // If not specified, the name of the log-file will be derived from the --ref and --in names
    if (!logo.length()) logo = path(obj) + filename(obj) + std::string("_to_") + filename(ref) + std::string(".log");
    return(logo);
  }
  const std::string& ObjOutFname() const {return(objo);}
  const std::string& RefOutFname() const {return(refo);}
  const std::string& FieldFname() const {return(fieldo);}
  const std::string& JacFname() const {return(jaco);}
  const std::string& IntensityMappingFname() const {return(into);}
  const std::string& ObjMask() const {return(objm);}
  const std::string& RefMask() const {return(refm);}
  const std::string& RefPointList() const {return(ref_pl);}
  const std::string& ObjPointList() const {return(obj_pl);}
  ...
  MISCMATHS::BFMatrixPrecisionType HessianPrecision() const {return(hess_prec);}
  ...
  const std::vector<unsigned int>& KnotSpacing(unsigned int lev) const {
  ...
  const std::vector<unsigned int>& DCTOrder(unsigned int lev) const {
  ...
  const std::vector<unsigned int> IntKnotSpacing(unsigned int lev) const {
  ...
  const std::vector<unsigned int> IntDCTOrder(unsigned int lev) const {
...
std::vector<std::shared_ptr<BASISFIELD::basisfield> > init_warpfield(const fnirt_clp&  clp);
...
std::shared_ptr<NEWIMAGE::volume<char> > make_mask(const std::string&              fname,
                                                   MaskType                        mt,
                                                   const NEWIMAGE::volume<float>&  ima,
                                                   bool                            impf,
                                                   double                          impv);

bool trying_to_register_to_self(const std::string&               ref_fname,
...
void set_nlpars(MISCMATHS::NonlinParam&  nlp);

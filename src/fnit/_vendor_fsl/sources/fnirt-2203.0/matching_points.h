...
public:
  ...
  virtual const char * what() const throw() {
    return(std::string("MatchingPoints:: " + m_msg).c_str());
  }
...
public:
  MatchingPoints(const NEWIMAGE::PointList&  points_in_ref, const NEWIMAGE::PointList& points_in_in)
  {
    ...
    _in_ref = std::shared_ptr<NEWIMAGE::PointList>(new NEWIMAGE::PointList(points_in_ref));
    _in_in = std::shared_ptr<NEWIMAGE::PointList>(new NEWIMAGE::PointList(points_in_in));
  }
  ...
  virtual std::shared_ptr<MISCMATHS::BFMatrix> SSD_Hessian(unsigned int                     dim,
                                                             const BASISFIELD::basisfield&    bf,
                                                             MISCMATHS::BFMatrixPrecisionType prec=MISCMATHS::BFMatrixDoublePrecision) const;

private:
  std::shared_ptr<NEWIMAGE::PointList>  _in_ref;
  std::shared_ptr<NEWIMAGE::PointList>  _in_in;
};

...
public:
  ...
  virtual const char * what() const throw() {
    return std::string("IntensityMapper:: msg=" + m_msg).c_str();
  }
...
public:
  ...
  IntensityMapper(const std::shared_ptr<BASISFIELD::basisfield>&  sfld,
  ...
  IntensityMapper(const std::vector<double>&                        sfac,
                  const std::shared_ptr<BASISFIELD::basisfield>&  sfld,
  ...
  IntensityMapper(const std::vector<std::shared_ptr<BASISFIELD::basisfield> >&   sfld,
  ...
  // respect to the parameters determining the scaling.
  virtual std::shared_ptr<MISCMATHS::BFMatrix> Hessian(const NEWIMAGE::volume<float>&    ref,
                                                         const NEWIMAGE::volume<char>      *mask,
                                                         MISCMATHS::BFMatrixPrecisionType  prec=MISCMATHS::BFMatrixDoublePrecision) const;
  ...
  virtual std::shared_ptr<MISCMATHS::BFMatrix> CrossHessian(const BASISFIELD::basisfield&        dfield,
...
protected:
  ...
  std::vector<std::shared_ptr<BASISFIELD::basisfield> >   _sfld;   // Vector of fields for local mapping
...
public:
  ...
  // Constructor for local linear mapping
  SSDIntensityMapper(std::shared_ptr<BASISFIELD::basisfield>&  sfld,
  ...
  // Constructor for local non-linear mapping
  SSDIntensityMapper(const std::vector<std::shared_ptr<BASISFIELD::basisfield> >&   sfld,
                     double                                                           lambda=1000.0) : IntensityMapper(sfld,lambda) {}
...
public:
  ...
  virtual const char * what() const throw() {
    return std::string("IntensityMapperReader:: msg=" + m_msg).c_str();
  }
...
public:
  ...
  std::vector<std::shared_ptr<BASISFIELD::basisfield> > GetLocalAsSplinefieldVector(const std::vector<unsigned int>&  ksp) const;

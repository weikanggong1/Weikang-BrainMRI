         ...
         char  *argv[])
{
  ...
  std::shared_ptr<volume<float> >     ref(new volume<float>);
  std::shared_ptr<volume<float> >     obj(new volume<float>);
  try {
  ...
  try {
    ...
    std::shared_ptr<volume<char> > objmask;
    ...
    std::shared_ptr<IntensityMapper>   mymapper = init_intensity_mapper(*clp);

    // Create cost-function object
    cf = std::shared_ptr<SSD_fnirt_CF>(new SSD_fnirt_CF(*ref,*obj,clp->Affine(),field,mymapper));
    ...
    nlpar = std::shared_ptr<NonlinParam>(new NonlinParam(cf->NPar(),clp->MinimisationMethod(),spar));
    set_nlpars(*nlpar);
  ...
  try {
    for (int ssl=2; ssl<=clp->NoLevels(); ssl++) {
      ...
      nlpar = std::shared_ptr<NonlinParam>(new NonlinParam(cf->NPar(),clp->MinimisationMethod(),cf->Par()));
      set_nlpars(*nlpar);

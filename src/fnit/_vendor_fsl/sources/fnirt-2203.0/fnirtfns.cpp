    ...
    verbose(pverbose.value()), debug(static_cast<unsigned int>(pdebug.value()))
{
  ...
  int64_t dim[] = {vref.xsize(), vref.ysize(), vref.zsize()};
...
std::shared_ptr<fnirt_clp> parse_fnirt_command_line(unsigned int   narg,
                                                      char           *args[])
{
  ...
  std::shared_ptr<fnirt_clp>   clp;
  try {
    clp = std::shared_ptr<fnirt_clp>(new fnirt_clp(refname,objname,affname,inwarpname,in_intname,coefname,imoutname,fieldoutname,
...
vector<std::shared_ptr<basisfield> > init_warpfield(const fnirt_clp&  clp)
{
  vector<std::shared_ptr<basisfield> >    field(3);
	...
	if (clp.RefSize() != reader.FieldSize()) throw fnirt_error(string("Field in file ")+clp.InWarp()+string(" not of same size as image in ")+clp.Ref());
        ...
        if (clp.Basis() == FNIRT::Spline) {
          ...
          for (unsigned int fi=0; fi<3; fi++) {
            field[fi] = std::shared_ptr<splinefield>(new splinefield(clp.RefSize(),clp.RefVxs(),clp.FullResKsp(),clp.SplineOrder()));
      ...
      for (int i=0; i<3; i++) field[i] = std::shared_ptr<splinefield>(new splinefield(reader.FieldAsSplinefield(i,clp.FullResKsp())));
    }
    else if (clp.Basis() == FNIRT::DCT) {
      for (int i=0; i<3; i++) field[i] = std::shared_ptr<dctfield>(new dctfield(reader.FieldAsDctfield(i,clp.DCTOrder(clp.NoLevels()))));
    }
  }
  else { // If we are starting from scratch
    if (clp.Basis() == FNIRT::Spline) {
      for (int i=0; i<3; i++) field[i] = std::shared_ptr<splinefield>(new splinefield(clp.RefSize(),clp.RefVxs(),clp.FullResKsp(),clp.SplineOrder()));
    }
    else if (clp.Basis() == FNIRT::DCT) {
      for (int i=0; i<3; i++) field[i] = std::shared_ptr<dctfield>(new dctfield(clp.RefSize(),clp.RefVxs(),clp.DCTOrder(clp.NoLevels())));
    }
  }
  return(field);
}

std::shared_ptr<IntensityMapper> init_intensity_mapper(const fnirt_clp&  clp)
{
  std::shared_ptr<IntensityMapper>  mymapper;  // Null pointer
  ...
  if (clp.IntMapType() == NONE) {
    mymapper = std::shared_ptr<IntensityMapper>(new SSDIntensityMapper);
  }
    ...
    mymapper = std::shared_ptr<IntensityMapper>(new SSDIntensityMapper(se));
  }
    ...
    std::shared_ptr<basisfield>  fld;  // Null-pointer
    if (clp.Basis() == Spline) {
      if (mapreader.HasLocal()) {
        if (mapreader.LocalFieldSize()!=clp.RefSize() || mapreader.LocalFieldVoxelSize()!=clp.RefVxs()) { // This means dimensions are not identical
          if ((std::fabs(mapreader.LocalFieldSize()[0]*mapreader.LocalFieldVoxelSize()[0] - clp.RefSize()[0]*clp.RefVxs()[0]) > 1e-3) ||
            ...
            fld = std::shared_ptr<BASISFIELD::basisfield>(new BASISFIELD::splinefield(clp.RefSize(),clp.RefVxs(),clp.FullResIntKsp()));
	  ...
	  if (mapreader.NLocalFields()==1) {
            fld = std::shared_ptr<basisfield>(new splinefield(mapreader.GetLocalAsSingleSplinefield(clp.FullResIntKsp())));
          }
          else if (mapreader.NLocalFields()>1) {
            fld = std::shared_ptr<basisfield>(new splinefield(mapreader.GetLocalAsSingleSplinefield(clp.FullResIntKsp(),1)));
	  }
	}
      }
      else {
        fld = std::shared_ptr<basisfield>(new splinefield(clp.RefSize(),clp.RefVxs(),clp.FullResIntKsp()));
    ...
    mymapper = std::shared_ptr<IntensityMapper>(new SSDIntensityMapper(fld,clp.IntLambda()));
  }
  else if (clp.IntMapType() == LOCAL_BIAS_WITH_GLOBAL_NON_LINEAR) {
    std::shared_ptr<BASISFIELD::basisfield>  fld;  // Null-pointer
    if (clp.Basis() == Spline) {
      if (mapreader.HasLocal()) {
        if (mapreader.LocalFieldSize()!=clp.RefSize() || mapreader.LocalFieldVoxelSize()!=clp.RefVxs()) { // This means dimensions are not identical
          if ((std::fabs(mapreader.LocalFieldSize()[0]*mapreader.LocalFieldVoxelSize()[0] - clp.RefSize()[0]*clp.RefVxs()[0]) > 1e-3) ||
    ...
    mymapper = std::shared_ptr<IntensityMapper>(new SSDIntensityMapper(se,fld,clp.IntLambda()));
  }
  else if (clp.IntMapType() == LOCAL_NON_LINEAR) {
    std::vector<std::shared_ptr<basisfield> >  fld(clp.IntMapOrder());
    if (clp.Basis() == Spline) {
      if (mapreader.HasLocal()) {
      }
      else {  // Brand new fields
        for (unsigned int i=0; i<clp.IntMapOrder(); i++) {
          fld[i] = std::shared_ptr<BASISFIELD::basisfield>(new BASISFIELD::splinefield(clp.RefSize(),clp.RefVxs(),clp.FullResIntKsp()));
...
void make_field_with_same_fov(const BASISFIELD::splinefield&                   in,
                              std::shared_ptr<BASISFIELD::basisfield>        out)
{
...
std::shared_ptr<NEWIMAGE::volume<char> > make_mask(const string&                  mfname,
                                                     MaskType                       mt,
                                                     const NEWIMAGE::volume<float>& ima,
                                                     bool                           impf,
                                                     double                         impv)
{
  std::shared_ptr<NEWIMAGE::volume<char> >  maskp;
  ...
  if (mfname.length() && mt!=IgnoreMask) { // If there is an explicit mask
    maskp = std::shared_ptr<NEWIMAGE::volume<char> >(new NEWIMAGE::volume<char>);
    ...
    maskp = std::shared_ptr<NEWIMAGE::volume<char> >(new NEWIMAGE::volume<char>(ima.xsize(),ima.ysize(),ima.zsize()));
                        ...
                        const NEWIMAGE::volume<float>&  ref)
{
  ...
  std::vector<std::shared_ptr<basisfield> >   field = init_warpfield(clp);
  std::shared_ptr<IntensityMapper>            intmap = init_intensity_mapper(clp);
  std::shared_ptr<SSD_fnirt_CF>   cf = std::shared_ptr<SSD_fnirt_CF>(new SSD_fnirt_CF(ref,ref,IdentityMatrix(4),field,intmap));

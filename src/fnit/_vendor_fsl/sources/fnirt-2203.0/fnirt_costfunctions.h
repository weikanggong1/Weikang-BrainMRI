...
public:
  ...
  virtual const char * what() const throw() {
    return std::string("Fnirt: msg=" + m_msg).c_str();
  }
...
public:
  ...
  fnirt_CF(const NEWIMAGE::volume<float>&                                 pvref,
           ...
           std::shared_ptr<IntensityMapper>                             im = std::shared_ptr<IntensityMapper>());

  // A little unsafe, but faster
  fnirt_CF(const std::shared_ptr<NEWIMAGE::volume<float> >              pvref,
  ...
  virtual void SetLambda(double plambda) {lambda = plambda; if (Verbose()) std::cout << "New Lambda: " << lambda << std::endl;}
  ...
  virtual void SetMatchingPoints(const MatchingPoints& pmpl) {mpl = std::shared_ptr<MatchingPoints>(new MatchingPoints(pmpl));}
  ...
  virtual void SetRefMask(const std::shared_ptr<NEWIMAGE::volume<char> > prefmask);
  virtual void SetObjMask(const std::shared_ptr<NEWIMAGE::volume<char> > pobjmask);
  ...
  virtual void SaveLocalIntensityMapping(std::string fname = std::string("fnirt_local_intensity_fields")) const;
  virtual void SaveGlobalIntensityMapping(std::string fname = std::string("fnirt_global_intensity_parameters.txt")) const;
  virtual void SaveIntensityMapping(std::string fname = std::string("fnirt_intensity_parameters")) const
  {
    SaveGlobalIntensityMapping(fname+std::string(".txt"));
...
protected:
  ...
  virtual const std::shared_ptr<IntensityMapper>& IMap() const {return(imap);}

  // Allow read-access to point-list mapper
  virtual const std::shared_ptr<MatchingPoints>& MPL() const {return(mpl);}

  // Set new value for intensity mapper
  virtual void SetIMap(const std::shared_ptr<IntensityMapper>&  new_imap) {imap = new_imap;}
  ...
  virtual MISCMATHS::BFMatrixPrecisionType HessianPrecision() const {return(hess_prec);}
...
private:
  ...
  const std::shared_ptr<NEWIMAGE::volume<float> >        vref;               // Reference volume
  std::shared_ptr<NEWIMAGE::volume<float> >              svref;              // Smoothed reference volume
  double                                                   ref_fwhm;           // FWHM of smoothed reference volume
  std::shared_ptr<NEWIMAGE::volume<float> >              ssvref;             // Subsampled smoothed reference volume
  std::vector<unsigned int>                                subsamp;            // Subsampling of subsampled reference volume

  const std::shared_ptr<NEWIMAGE::volume<float> >        vobj;       // Object volume, i.e. volume to be warped to vref
  std::shared_ptr<NEWIMAGE::volume<float> >              svobj;      // Smoothed object volume
  ...
  MISCMATHS::BFMatrixPrecisionType                         hess_prec;          // Can be float or double
  ...
  std::shared_ptr<NEWIMAGE::volume<char> >    refmask;                       // Mask in space of reference volume
  std::shared_ptr<NEWIMAGE::volume<char> >    ssrefmask;                     // Mask in sub-sampled reference volume
  std::shared_ptr<NEWIMAGE::volume<char> >    objmask;                       // Mask in space of object volume

  // Scratch pads

  mutable std::shared_ptr<NEWIMAGE::volume<float> >   robj;                // Resampled version of object volume
  mutable std::shared_ptr<NEWIMAGE::volume<char> >    datamask;            // One where robj reflects "true" interpolated intensity
  mutable bool                                          robj_updated;        // True if robj accurately reflects df
  mutable std::shared_ptr<NEWIMAGE::volume4D<float> > df;                  // Volume representation of displacement field
  mutable std::shared_ptr<NEWIMAGE::volume<char> >    robjmask;            // Resampled version of objmask
  mutable bool                                          robjmask_updated;    // True if robjmask accurately reflects df
  mutable std::shared_ptr<NEWIMAGE::volume<char> >    totmask;             // Total mask, only used if refmask and/or objmask is set.
  mutable bool                                          totmask_updated;
  mutable std::shared_ptr<NEWIMAGE::volume4D<float> > robj_deriv;          // Derivative of object image at resampled points.
  mutable bool                                          robj_deriv_updated;  // True if derivatives accurately reflects current df
  mutable std::shared_ptr<NEWIMAGE::volume4D<float> > ref_deriv;           // Derivative of reference image in native space
  ...
  std::shared_ptr<NEWIMAGE::volume<float> > masked_smoothing(const NEWIMAGE::volume<float>&              vol,
                                                               double                                      fwhm,
  ...
  void common_construction(std::vector<std::shared_ptr<BASISFIELD::basisfield> >& pdf_field);
...
public:
  ...
  {
    last_hess = std::shared_ptr<MISCMATHS::BFMatrix>();   // NULL
    if (!im) {                                   // Linear scaling default for ssd
      std::shared_ptr<IntensityMapper>  def_imap(new SSDIntensityMapper(1.0));
      SetIMap(def_imap);
    }
  }

  // Unsafe (if you misbehave), but faster.
  SSD_fnirt_CF(const std::shared_ptr<NEWIMAGE::volume<float> >         pvref,
               const std::shared_ptr<NEWIMAGE::volume<float> >         pvobj,
               const NEWMAT::Matrix                                    aff,
               std::vector<std::shared_ptr<BASISFIELD::basisfield> >&  pdf_field,
               std::shared_ptr<IntensityMapper>                        im = std::shared_ptr<IntensityMapper>())
...
private:
  ...
  mutable std::shared_ptr<MISCMATHS::BFMatrix>                   last_hess;
  // Internal copy of ptr-> regularisation part of Hessian. Saves us a little time
  mutable std::shared_ptr<MISCMATHS::BFMatrix>                   reg;
};

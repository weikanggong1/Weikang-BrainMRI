...
std::vector<std::shared_ptr<NEWMAT::ColumnVector> > basisfield::get(const std::vector<FieldIndex>& fiv)
{
  if (!coef) { throw BasisfieldException("basisfield::Get: Coefficients undefined"); }
  if (std::any_of(fiv.cbegin(),fiv.cend(),[this](const FieldIndex& fi){ return(!this->UpToDate(fi)); })) {
    Update(fiv);
  }
  std::vector<std::shared_ptr<NEWMAT::ColumnVector> > rfields(fiv.size());
  for (unsigned int i=0; i<fiv.size(); i++) rfields[i] = field[fiv[i]];
  return(rfields);
}

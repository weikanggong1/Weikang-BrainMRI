...
private:
  std::string m_msg;
public:
  BasisfieldException(const std::string& msg) noexcept : m_msg(std::string("BasisField:: msg=" + msg)) {}
...
protected:
  ...
  // Get smart read/write pointers to updated fields
  virtual std::vector<std::shared_ptr<NEWMAT::ColumnVector> > get(const std::vector<FieldIndex>& fi);
...
public:
  ...
  virtual std::shared_ptr<NEWMAT::ColumnVector> GetCoef() const {return(get_coef());}
  ...
  virtual std::shared_ptr<NEWMAT::ColumnVector> Get(FieldIndex fi=FIELD) const { 
    return(const_cast<BASISFIELD::basisfield *>(this)->get(fi)); 
  }
  virtual std::vector<std::shared_ptr<NEWMAT::ColumnVector> > Get(const std::vector<FieldIndex>& fiv) const { 
    return(const_cast<BASISFIELD::basisfield *>(this)->get(fiv)); 
  }
  ...
  virtual void SetNthreads(unsigned int nthr) = 0;
  ...
  virtual void Update(const std::vector<FieldIndex>& fiv) = 0;

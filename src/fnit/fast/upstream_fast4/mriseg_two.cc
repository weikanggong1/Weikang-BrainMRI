...
/*For a segmentation of nClasses, this returns the constituents for a given
binary mixel type. The allowed mixel types are sequentially adjacent classes
( 1,2 2,3 ... nClasses-1,nClasses ) with the exception of the 3-class
segmentation, which allows all combinations in the sequence ( 1,2 1,3 2,3 ).

Confusingly, mixeltype ( hardPV ) is coded 0...max(type), where 0..nClasses-1
are the homogeneous mixels, but most class containers are coded 1..nClasses,
with the 0th element being unused. The coding returned by this function is for
these containers, and _not_ hardPV.*/

pair<unsigned int,unsigned int> mixelType(const int nClasses, const int type) {
  if ( type > ( nClasses == 3 ? 5 : ( 2*nClasses ) -1) )
    throw std::invalid_argument("Mixeltype exceeds allowed types");
  if ( type < 0 )
    throw std::invalid_argument("Mixeltype cannot be negative");

	pair<unsigned int,unsigned int> mixelTypes(type-nClasses+1,type-nClasses+2);
	if( nClasses == 3 && type == 4 ) {
		mixelTypes.first = 1;
		mixelTypes.second = 3;
	}
	if( nClasses == 3 && type == 5 ) {
		mixelTypes.first = 2;
		mixelTypes.second = 3;
	}
	return mixelTypes;
		  ...
		  for(int x=0;x<m_nWidth;x++)
	     	if( m_mask.value(x, y, z) > 0 ) {
		     	if( hardPV(x, y, z)<nClasses ) {
			 			m_pve(x, y, z, hardPV(x, y, z)+1) = 1.0;
					}
          else {
						float min(1.0e13);
						for(float delta=0.00;delta<=1.0;delta+=step) {
			     		float mu (
								delta*m_mean[mixelType(nClasses,hardPV(x,y,z)).first]+(1-delta)*m_mean[mixelType(nClasses,hardPV(x,y,z)).second]
							);
							float sigsq (
								delta*delta*m_variance[mixelType(nClasses,hardPV(x,y,z)).first]+(1-delta)*(1-delta)*m_variance[mixelType(nClasses,hardPV(x,y,z)).second]
							);
			     			float val( PVEnergy(x, y, z, mu, sigsq) );
			     			if( min > val ) {
				 				  m_pve(x, y, z, mixelType(nClasses,hardPV(x,y,z)).first) = delta;
				 				  m_pve(x, y, z, mixelType(nClasses,hardPV(x,y,z)).second)= 1.0-delta;
				 				  min=val;
			       		}
			   		}
		      }
